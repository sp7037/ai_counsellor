...
new #[Layout('components.layouts.tenant')] class extends Component {
    ...
    public function with(LeadDirectoryService $directory, CounsellorManagementService $counsellors): array
    {
        return [
            ...
            'provisioningBlockReason' => $counsellors->provisioningBlockReason($this->tenant),
        ];
    }
}; ?>
...
<x-slot:actions>
    @unless ($provisioningBlockReason)
        <flux:button href="{{ route('tenant.counsellors.create', $tenant) }}" wire:navigate variant="primary" size="sm">Add counsellor</flux:button>
    @endunless
</x-slot:actions>
...
<div class="grid gap-4">
    ...
    @if ($provisioningBlockReason)
        <flux:callout variant="warning">{{ $provisioningBlockReason }}</flux:callout>
    @endif
    ...
    <div class="overflow-x-auto rounded-lg border border-zinc-800">
        <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y divide-zinc-800 text-zinc-200">
                @forelse ($counsellors as $membership)
                ...
                @empty
                    <tr><td colspan="4" class="px-4 py-10 text-center text-zinc-500">No counsellors yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
